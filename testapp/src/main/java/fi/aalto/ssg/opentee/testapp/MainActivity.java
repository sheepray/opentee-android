...
;
    ...
    void newTest(){
        ...
        try {
            client.initializeContext(TEE_NAME, getApplication());
    ...
    void test(){
        ...
        if ( session == null ){
            Log.d(TAG, "session not ready");
        }
        else{
            Log.d(TAG, "coming operations related with session should be put in here. For the purpose" +
                    "of testing, the following operations related with session are put outside of this" +
                    "{}");
        }
